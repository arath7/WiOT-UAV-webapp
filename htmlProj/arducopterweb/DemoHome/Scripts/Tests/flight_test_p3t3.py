...
DC_awake_period_in_milliseconds = 1000				# Duty cycle awake duration. Must be greater than or equal to 0, and less than or equal to DC_cycle_period_in_milliseconds.
flight_altitude_in_metres = 3						# Expected altitude of the drone, in metres. Must be greater than or equal to 1.
...
class PacketTypes( Enum ):
	SEND_CTS = 7		# MDC_Sniffer tells SN_WuR to send a data packet to MDC_Sniffer.
	SEND_WUS = 8		# MDC_Sniffer tells MDC_WuR to broadcast Wake-up Sequence, which in turn tells SN_WuR to send a data packet to MDC_Sniffer.
...
if not debug_comm:
	class MDCn_AM_Packet( tos.Packet ):
...
assert 25 <= DC_cycle_period_in_milliseconds
...
## Instruct the MDC_Sniffer to send a packet to the MDC_WuR that will result in a WuS being broadcast.
def send_request_for_data_packet( active_message_controller , destination_SN ):
	if active_message_controller != None:
		packet_to_send = MDCn_AM_Packet()
		packet_to_send.source = Nodes.MDC_SNIFFER.value
		if scenario_WuR:
			packet_to_send.destination = Nodes.MDC_WUR.value
			packet_to_send.packet_type = PacketTypes.SEND_WUS.value
		else:
			packet_to_send.destination = destination_SN
			packet_to_send.packet_type = PacketTypes.SEND_CTS.value
		...
		try:
			...
			if scenario_WuR:
				print( get_EST() , '|' , 'Error: failed to send the SEND_WUS packet.' , flush = True )
			else:
				print( get_EST() , '|' , 'Error: failed to send the SEND_CTS packet.' , flush = True )
			if verbose:
				print( e , flush = True )
			print( flush = True )
	else:
		print( get_EST() , '|' , 'Error: Invalid active message specified.' , flush = True )
...
def receive_serial_packet( active_message_controller ):
	...
	if active_message_controller != None:
		try:
			...
			pass
		if received_message:
			received_packet = MDCn_AM_Packet( received_message.data )
	else:
		print( '\n' + 'Error: Invalid active message specified.' + '\n' , flush = True )
	return received_packet

## The MDC_Sniffer sends a packet to the MDC_WuR, which broadcasts a WuS, which tells an SN to send back a data packet.
def collect_data_from_node( active_message_controller , SN_ID ):
	...
	while loop and ( active_message_controller != None ):
		...
		if received_packet != None:
			...
			if received_packet.packet_type == PacketTypes.DATA.value:
...
def run_collection_cycle( active_message_controller , drone ):
	...
	if ( active_message_controller or debug_comm ) and ( drone or debug_flight ):
		...
		collection_points = scale_waypoints_by_factor( collection_points , 1/4 , origin )
		...
		for collection_point in collection_points[ 1: ]:
...
def finalise_collection_cycle( active_message_controller ):
	...
	awake_durations = [ None for _ in range( len( latencies ) ) ]
